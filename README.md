...
gives the full reasoning and the alternatives that were rejected.
...
that record's PR is the ratification record.
...
mean the design meets them — three rows are recorded as missed above, and two
rows (offset, noise) rest on lower-bound measurements that cannot certify
compliance either way until the transient Monte-Carlo and transient-noise
benches DR-0001 named actually exist. Nothing here is post-layout; no layout
exists.
