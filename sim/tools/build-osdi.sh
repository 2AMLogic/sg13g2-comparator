...
#
# Ported from sg13g2-opamp's sim/tools/build-osdi.sh (issue #8's Scope
# section names it as the source), replacing the manual openvaf-r step
# previously documented in sim/device-mismatch-confirm/README.md
# "Reproducing". Only the cache-directory default and repo-name references
# changed; the checksum pins and the platform-specific workarounds are
# copied verbatim -- they are properties of the OpenVAF-Reloaded release and
# this PDK's Verilog-A sources, not of which repo is doing the building.
#
...
# them, every MOS and every resistor in any netlist under sim/ is
# unsimulatable -- see sim/harness/README.md "OSDI preflight" for the full
# account.
#
...
# --------------------------------------------------------------------------
# Pins. These are the single source of truth; sim/toolchain.json's
# "openvaf_tag" field restates the tag (not the sha256s) as a greppable fact
# sheet (that file is documentation, not consumed by tooling). Keep the two
# in sync.
# --------------------------------------------------------------------------
...
# Linux/x86_64 only: the pinned openvaf-r release above ships
...
# for our own openvaf-r invocations. See
...
check_models() {
  ...
  trap 'rm -rf "${tmp}"' RETURN
  {
    ...
    echo ".lib \"${PDK_ROOT}/${PDK}/libs.tech/ngspice/models/cornerMOSlv.lib\" mos_tt"
    echo ".lib \"${PDK_ROOT}/${PDK}/libs.tech/ngspice/models/cornerRES.lib\" res_typ"
    echo "Vdd vdd 0 dc 1.2"
    echo "Vg g 0 dc 0.6"
    echo "XMP d1 g vdd vdd sg13_lv_pmos w=10u l=1u ng=1 m=1"
    echo "Rd1 d1 0 100k"
    echo "XMN d2 g 0 0 sg13_lv_nmos w=2u l=0.5u ng=1 m=1"
...
# Persistent cache, NOT /tmp: re-downloading a ~60 MB compiler every session
# is avoidable churn. Override with SG13G2_TOOLS_CACHE.
CACHE_DIR="${SG13G2_TOOLS_CACHE:-${HOME}/.cache/sg13g2-comparator}"
mkdir -p "${CACHE_DIR}"
