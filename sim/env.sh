...
# /usr/local/share/pdk, ~/share/pdk, ~/.ciel, ~/.volare), so every
# sim/*/run_*.sh script agrees on which PDK install is in use. Mirrors
# sg13g2-bandgap's sim/env.sh convention (same PDK, same resolution order).
# Safe to source from any directory; does not require `klt` to be
# installed (testbenches under sim/ must remain runnable in a sandbox that
...
if [[ -n "${PDK_ROOT:-}" && -d "${PDK_ROOT}/${PDK}/libs.tech/ngspice" ]]; then
  ...
  # OSDI-compiled Verilog-A device models (PSP103 MOS). This directory is
  # where the PDK's own install expects them and does NOT exist in a
  # freshly-unpacked IHP-Open-PDK v0.3.0 tree (that release ships
  # Verilog-A sources but no prebuilt .osdi binaries). Every testbench
  # that instantiates an sg13_lv_nmos/sg13_lv_pmos loads psp103.osdi from
  # here via `pre_osdi`.
  ...
  if [[ ! -f "${SG13G2_OSDI_DIR}/psp103.osdi" ]]; then
    echo "sg13g2: OSDI device model psp103.osdi not built yet in ${SG13G2_OSDI_DIR}" >&2
    echo "sg13g2: LV MOS devices (sg13_lv_nmos/sg13_lv_pmos) will not simulate" >&2
    echo "sg13g2: until it is. See sim/device-mismatch-confirm/README.md 'Reproducing'" >&2
    echo "sg13g2: for the build recipe (same one sg13g2-bandgap's sim/tools/build-osdi.sh uses)." >&2
  fi
else
